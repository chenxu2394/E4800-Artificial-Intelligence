...
def unit_propagation(valuation, clauses):
    ...
    * Otherwise, if all literals of a clause are False, then the whole clause
    ...
    queue = set()  # set of literals
    while True:
        for c in clauses:
            noAssignCount = 0
            for literal in c:
                if literal[0] in valuation:
                    if valuation[literal[0]] != literal[1]:
                        continue
                    else:  # found one true literal
                        break
                else:
                    oneLiteral = literal
                    noAssignCount += 1
                ...
                if noAssignCount == 0:
                    return False, {}
                elif noAssignCount == 1:
                    queue.add(oneLiteral)


            # c_dict = {tup[0]: tup[1] for tup in c}
            # atomSet = set(c_dict.keys())
            # valSet = set(valuation.keys())
            # diffSet = atomSet - valSet
            # unionSet = atomSet & valSet
            # if atomSet == valSet and all(valuation[element] != c_dict[element] for element in unionSet):
            #     return False, {}
            # if len(diffSet) == 1 and all(valuation[element] != c_dict[element] for element in unionSet):
            #     el, = diffSet
            #     queue.add((el, c_dict[el]))
        if not bool(queue):
            return True, valuation

        literal = queue.pop()
        if literal[0] in valuation.keys() and valuation[literal[0]] != literal[1]:
            return False, {}
        valuation[literal[0]] = literal[1]
...
def dpll(valuation, clauses, variables):
    ...
    result = dpll(valuation, clauses, variables)
    if result[0]:
        return result
    else:
...
if __name__ == '__main__':
    ...
    print(valuation)
    print(clauses)
    # print(dpll(valuation, clauses, ['a', 'b', 'd', 'e', 'f']))
    # print(dpll(valuation, clauses, ['a']))
