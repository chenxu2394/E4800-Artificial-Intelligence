...
def construct_probability_table(data, query, conditions):
    ...
    for assignment in assignments:
        ...
        # =========================================================
        prob = get_conditional_probability(data, query=A, conditions=B)
        result[dict_key] = prob
...
def exhaustive_enum(G, X, E, e):
    ...
    for assignment in assignments:
        ...
        for node in G.nodes:
            ...
            #     node_parents[parent_node.data_id] = combined_dictionary[parent_node.data_id]
            tmp *= evaluate_node(G=G, query=node.data_id, val=combined_dictionary[node.data_id],
                                 conditions=combined_dictionary)
...
def sample(G, X, E, e):
    ...
    for variable in X:
        ...
        if variable in E:
            ...
            temp = G.get_node_probability(feature=variable, conditions=parents)
            if value:
                w *= temp
            else:
                w *= (1-temp)
