...
def main():
    ...
    # players = [IDMinimaxAgent, IDAlphaBetaAgent]
    ...
    results = {(player_name(players[0]), 0): 0,
               (player_name(players[0]), 1): 0,
               (player_name(players[1]), 0): 0,
               (player_name(players[1]), 1): 0,
               }
    ...
    for i in range(150):
        ...
        winners = game.play(initial_state, output=True, visualizer=None, timeout_per_turn=timeouts)
        # winners = game.play(initial_state, output=True, visualizer=visualizer, timeout_per_turn=timeouts)
        if len(winners) == 1:
            results[(initial_state.get_players_names()[winners[0]], winners[0])] += 1
            # print(winners)
        print("")
        print(f"{i}) Result: {player_name(players[0])} 1st:{results[(player_name(players[0]), 0)]} - {player_name(players[1])} 1st:{results[(player_name(players[1]),0)]}")
        print(f"{i}) Result: {player_name(players[0])} 2nd:{results[(player_name(players[0]), 1)]} - {player_name(players[1])} 2nd:{results[(player_name(players[1]),1)]}")
        print("########################################################")
