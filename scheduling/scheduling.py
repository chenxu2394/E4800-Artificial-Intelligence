...
def at_most_one(formulas):
    ...
    # return EQVI(AND([NOT(AND([f1, f2])) for (f1, f2) in all_pairs(formulas)]), True)
...
def exactly_one_execution(tasks, time_interval):
    ...
    return AND([exactly_one([variable(task, time_slot) for time_slot in range(time_interval[0], time_interval[1])]) for task in tasks])
...
def mutually_exclusive_tasks(tasks, resource_need, time_interval):
    ...
    resource_dict = {}
    for task in tasks:
        resource = resource_need[task]
        if resource not in resource_dict.keys():
            resource_dict[resource] = [task]
        else:
            resource_dict[resource].append(task)
    # print(resource_dict)
    return AND([at_most_one([variable(task, time_slot) for task in resource_dict[resource]]) for resource in resource_dict.keys() for time_slot in range(time_interval[0], time_interval[1])])
...
def task_ordering(orders, time_interval):
    ...
    return AND([NOT(AND([variable(task1, t1), variable(task2, t2)])) for t1 in range(time_interval[0], time_interval[1]) for t2 in range(time_interval[0], t1+1) for task1, task2 in orders])
