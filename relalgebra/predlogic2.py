...
class NOT:
    ...
    def toRelAlg(self,dbSchema):
        ...
        # exit(1)
        vars_ = self.subformula.freeVars()
        formula = reduce(AND, [ ATOM("U",[Var(x)]) for x in vars_])
        return RAdifference(formula.toRelAlg(dbSchema), self.subformula.toRelAlg(dbSchema))
###### IMPLEMENT THIS
###### IMPLEMENT THIS
...
class FORALL:
    ...
    def toRelAlg(self,dbschema):
        if self.var not in self.subformula.freeVars():
            ...
            # exit(1)
            universe_formula = ATOM("U",[Var(self.var)])
            return RAdivision(self.subformula.toRelAlg(dbschema), universe_formula.toRelAlg(dbschema))
###### IMPLEMENT THIS
###### IMPLEMENT THIS
