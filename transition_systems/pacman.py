    ...
    def successors(self):
        ...
        if self.d == 'N':
            if not self.grid.occupied(self.x, self.y+1):
                res.append(('forward', PacManState(self.x, self.y+1, self.d, self.grid)))
            if not self.grid.occupied(self.x-1, self.y):
                res.append(('left', PacManState(self.x-1, self.y, 'W', self.grid)))
            if not self.grid.occupied(self.x+1, self.y):
                res.append(('right', PacManState(self.x+1, self.y, 'E', self.grid)))
            if not bool(res):
                res.append(('backward', PacManState(self.x, self.y-1, 'S', self.grid)))
            return res
        elif self.d == 'S':
            if not self.grid.occupied(self.x, self.y-1):
                res.append(('forward', PacManState(self.x, self.y-1, self.d, self.grid)))
            if not self.grid.occupied(self.x+1, self.y):
                res.append(('left', PacManState(self.x+1, self.y, 'E', self.grid)))
            if not self.grid.occupied(self.x-1, self.y):
                res.append(('right', PacManState(self.x-1, self.y, 'W', self.grid)))
            if not bool(res):
                res.append(('backward', PacManState(self.x, self.y+1, 'N', self.grid)))
            return res
        elif self.d == 'E':
            if not self.grid.occupied(self.x+1, self.y):
                res.append(('forward', PacManState(self.x+1, self.y, self.d, self.grid)))
            if not self.grid.occupied(self.x, self.y+1):
                res.append(('left', PacManState(self.x, self.y+1, 'N', self.grid)))
            if not self.grid.occupied(self.x, self.y-1):
                res.append(('right', PacManState(self.x, self.y-1, 'S', self.grid)))
            if not bool(res):
                res.append(('backward', PacManState(self.x-1, self.y, 'W', self.grid)))
            return res
        elif self.d == 'W':
            if not self.grid.occupied(self.x-1, self.y):
                res.append(('forward', PacManState(self.x-1, self.y, self.d, self.grid)))
            if not self.grid.occupied(self.x, self.y-1):
                res.append(('left', PacManState(self.x, self.y-1, 'S', self.grid)))
            if not self.grid.occupied(self.x, self.y+1):
                res.append(('right', PacManState(self.x, self.y+1, 'N', self.grid)))
            if not bool(res):
                res.append(('backward', PacManState(self.x+1, self.y, 'E', self.grid)))
            return res
        else:
