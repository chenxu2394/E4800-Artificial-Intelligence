...
def value_of(mdp, s, a, v, gamma):
    ...
    result = 0.
    for s_ in mdp.successor_states(s=s, a=a):
        result += mdp.P(s1=s, a=a, s2=s_) * (mdp.R(s1=s, a=a, s2=s_) + gamma * v[s_])
    return result
    ...
    while True:
        ...
        for s in mdp.states():
            for a in mdp.applicable_actions(s):
                temp = value_of(mdp=mdp, s=s, a=a, v=v_old, gamma=gamma)
                if temp > v_new[s]:
                    v_new[s] = temp
        if all(abs(v_new[s] - v_old[s]) < error for s in mdp.states()):
            break
        else:
            v_old = v_new.copy()
...
if __name__ == "__main__":
    # Very basic examples.
    #
    # For more and tests with automatic comparison run test_valueiteration.py
