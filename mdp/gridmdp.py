...
class GridMDP(mdp.MDP):
    ...
    def __init__(self, terrain, wall_tiles={'#'},
                 tile_rewards={'+': 1, '-': -1},
                 no_move_reward=-1):
        """
        ...
        tile_rewards : dictionary string : float
           ...
           will have default reward 0.
        no_move_reward : float, default: -1
           Constant penalty/reward associated with remaining in the same spot,
           whether intentionally (through the remain action) or unintentionally
           (e.g. by attempting to move into all wall). The reward is calculated
           as <tile reward> + <no_move_reward> in the case of a no move.
        ...
        self._no_move_reward = no_move_reward;
    ...
    def R(self, s1, a, s2):
        ...
        r = self._rewards.get(t, 0.0)
        # Check if no move should be applied.
        if s2 == s1:
            r += self._no_move_reward
        return r
